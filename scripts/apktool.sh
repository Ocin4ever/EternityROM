...
BUILD()
{
    ...
    if [ -d "$OUTPUT_PATH/smali" ]; then
        ...
        for f in "$OUTPUT_PATH/smali"*; do
            ...
            EVAL "smali a -a \"$DEX_API_LEVEL\" -o \"$OUTPUT_PATH/$DEX_FILENAME\" \"$f\"" &
        done
    ...
    EVAL "apktool b -p \"$FRAMEWORK_DIR\" -srp \"$OUTPUT_PATH\"" || exit 1
...
DECODE()
{
    ...
    EVAL "apktool d -o \"$OUTPUT_PATH\" -p \"$FRAMEWORK_DIR\" -s \"$INPUT_FILE\"" || exit 1
    ...
    if [ -f "$OUTPUT_PATH/classes.dex" ]; then
        ...
        for f in "$OUTPUT_PATH/"*.dex; do
            ...
            EVAL "baksmali d -a \"$DEX_API_LEVEL\" --ac false --di false -l -o \"$OUTPUT_PATH/$SMALI_OUT\" --sl \"$f\"" &
        done
